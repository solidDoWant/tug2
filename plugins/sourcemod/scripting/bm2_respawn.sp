...
//     if(db == null){
//         LogError("[GG Respawn] T_Connect returned invalid Database Handle");
//         SetFailState("FAILED TO CONNECT TO M DB, BAILING");
//         return;
//     }
//     g_Database = db;
//     LogMessage("[GG Respawn] Connected to Database.");
//     return;
// } 
...
public void do_nothing(Handle owner, Handle results, const char[] error, any client) {
    if (strlen(error) != 0) {
        ...
        if (StrContains(error, "UNIQUE constraint failed", false) != 0) {
            ...
            LogMessage("[GG Respawn] error: %s", error);
        }
    }
    return;
}

public void addRevive(int client) {
	// static char steamId[64];
	// GetClientAuthId(client, AuthId_SteamID64, steamId, sizeof(steamId));

	// char query[256];

	// Format(query, sizeof(query), "UPDATE players SET revives = revives + 1 WHERE steamId = '%s' AND server_id = '%i'", steamId, g_server_id.IntValue);
	// SQL_TQuery(g_Database, do_nothing, query, client);

	update_revive_count(client);
	//LogMessage("[RESPAWN] Adding revive to db for %N", client);
}

public void addHeal(int client, int val) {
	// static char steamId[64];
	// GetClientAuthId(client, AuthId_SteamID64, steamId, sizeof(steamId));

	// char query[256];
	// Format(query, sizeof(query), "UPDATE players SET healed_hp = healed_hp + %i WHERE steamId = '%s' AND server_id = '%i'", val, steamId, g_server_id.IntValue);
	// SQL_TQuery(g_Database, do_nothing, query, client);
	update_healed_hp_count(client, val);
	//LogMessage("[RESPAWN] Adding healed_hp to db for %N", client);
}
...
Action BotsReady_Timer(Handle timer) {
	if (g_TeamSecCount > 0) {		//Must check it because a player can glitch it by joining the spectator team.
		g_botsReady = 1;
	}
	else {
		g_iRoundStatus = 0;
...
public Action Event_ControlPointCaptured_Pre(Event event, const char[] name, bool dontBroadcast) {
	...
	if (g_iACP == g_iNCP) {	// Final counter attack
...
public Action Event_ObjectDestroyed_Pre(Event event, const char[] name, bool dontBroadcast) {
	...
	// Set counter attack duration to server
	if (g_iACP + 1 == g_iNCP) {	// Final counter attack
	...
	float fRandom = GetRandomFloat(0.0, 1.0);	// Get ramdom value for occuring counter attack
	if (fRandom < g_fCounterChance && g_iACP != g_iNCP) {	// Occurs counter attack
		SetConVarInt(g_cvCounterattackDisable, 0, true, false);
		SetConVarInt(g_cvCounterattackAlways, 1, true, false);
		CreateTimer((g_fCounterattackDuration + 1.0), Timer_CounterAttackEnd);
...
Action Timer_CounterAttackEnd(Handle timer) {	// When counter-attack end, reset reinforcement time
	g_iTimerReinforceTime = g_iReinforceTime;	// Reset reinforcement time
	ResetInsurgencyLives();	// Reset respawn token
...
public Action Event_PlayerHurt_Pre(Event event, const char[] name, bool dontBroadcast) {
	...
	if (!g_iHurtFatal[victim])	{	//Track wound type (minor, moderate, critical)
...
void RemoveRagdoll(int client) {	// Remove ragdoll
...
void CreateReviveTimer(int client) {	// This handles revives by medics
...
#if DOCTOR
Action RespawnPlayerRevive(Handle timer, int client) {	// Revive player
	if (!IsClientInGame(client)) {
		return Plugin_Stop;
	}
...
Action RespawnBot(Handle timer, int client) {
	...
	if (StrEqual(sModelName, "")) {	//check if model is blank
		return Plugin_Stop;
	}
...
public Action SendForwardMedicRevive(int iMedic, int iInjured) {	// tug stats forward
	Action result;
...
public Action SendForwardMedicHealed(int iMedic, int iInjured) {	// tug stats forward
	Action result;
...
public Action SendForwardResult(int iMedic, int iInjured, int revive) {	// tug stats forward
	Action result;
	Call_StartForward(MedicRevivedForward);
	Call_PushCell(iMedic);
	Call_PushCell(iInjured);
...
char[] GetDistanceString(float fDistance) {	// Return distance string
	float fTempDistance = fDistance * 0.01905;		// Distance to meters
	char sResult[64];
	if (g_iReviveDistanceMetric) {	// Distance to feet?
...
int GetTeamSecCount() {	// Get tesm2 player count
	int clients = 0, iTeam;
	for ( int i = 1; i <= MaxClients; i++ ) {
		if (IsClientInGame(i)) {
...
#if DOCTOR
int TraceClientViewEntity(int client) {	// Trace client's view entity
...
bool TRDontHitSelf(int entity, int mask, any data) { // Don't ray trace ourselves -_-"		// Check is hit self
