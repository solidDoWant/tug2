...
ConVar    g_cvRecentMaps      = null;

int       ga_iCooldown[MAXPLAYERS + 1];
int       g_iRecentMapsToExcludeCount;

char      g_sDefMapcycleFile[PLATFORM_MAX_PATH];
char      g_sTempMapcycleFile[PLATFORM_MAX_PATH];

bool      g_bHooked            = false;
bool      g_bLoadArrayFromFile = true;

ArrayList ga_hExcludedMaps;

public Plugin myinfo =
{
    name        = "[GG2 map changeups // lastmaps]",
    author      = "Nullifidian",
...
public void OnPluginStart()
{
    ...
    g_iRecentMapsToExcludeCount = g_cvRecentMaps.IntValue;
    ...
    AutoExecConfig(true, "map_changeups");
}
...
{
    if (ga_hExcludedMaps.Length == (g_iRecentMapsToExcludeCount + 1))
    {
        ga_hExcludedMaps.Erase(0);
        return;
    }

    if (ga_hExcludedMaps.Length < (g_iRecentMapsToExcludeCount + 1)) return;

    for (int i = 0; i < (ga_hExcludedMaps.Length - g_iRecentMapsToExcludeCount); i++)
    {
        ga_hExcludedMaps.Erase(0);
    }
}
...
void ArrayToFile()
{
    if (ga_hExcludedMaps.Length < 1) return;
    ...
    for (int i = 0; i < ga_hExcludedMaps.Length; i++)
    {
        if (i > g_iRecentMapsToExcludeCount) break;
...
void ArrayFromFile(const char[] sMap)
{
    ...
    if (hRead)
    {
        ...
        while (!IsEndOfFile(hRead))
        {
            if (!ReadFileLine(hRead, sBuffer, sizeof(sBuffer))) continue;
            ...
            if (strcmp(sBreak, sMap, false) == 0) continue;

            ga_hExcludedMaps.PushString(sBuffer);

            if ((ga_hExcludedMaps.Length - 1) >= g_iRecentMapsToExcludeCount && g_iRecentMapsToExcludeCount > 0)
            {
                ga_hExcludedMaps.Erase(0);
            }
        }
