...
public void Event_GameStart(Handle event, const char[] name, bool dontBroadcast)
{
    remove_restrictedarea();
}

public void remove_restrictedarea()
{
    ...
    for (int i = 0; i <= GetMaxEntities(); i++)
    {
        ...
        if (!GetEdictClassname(i, name, sizeof(name))) continue;
        if (!StrEqual("ins_blockzone", name, false)) continue;
