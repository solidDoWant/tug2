...
# Advertises the fastdl content to clients. The list is fetched from the fastdl host at runtime
# rather than baked in, so this needs ripext for HTTP + JSON. See the plugin header for why.
RUN --mount=type=bind,source=./plugins/sourcemod/scripting,target=/plugin-source/scripting \
    --mount=from=sourcemod-extensions-ripext,source=/insurgency/addons/sourcemod/scripting/include,target=/ripext-include \
    /sourcemod/addons/sourcemod/scripting/spcomp --include=/plugin-source/scripting/include --include=/ripext-include /plugin-source/scripting/gg2_fastdl.sp -o /insurgency/addons/sourcemod/plugins/gg2_fastdl.smx && \
    find /insurgency -type d -exec chmod 755 {} \; && \
    find /insurgency -type f -exec chmod 644 {} \;
...
# fastdl host is actually serving.
#
# THIS HAS TO BE A COMMAND-LINE ARG, not a line in server.cfg or a playlist forced_cvar. Both of
# those are re-applied on every map change, before the theater loads, so either would clobber the
# plugin's value every single map. The command line is applied once at startup.
#
# Empty means "leave the engine default", which is what servers that set the cvar in their own
# server.cfg (main) want - their config runs after this and wins.
ENV THEATER_NAME=
...
# The theater this image ships, used until gg2_fastdl has read the fastdl manifest and repointed the
# cvar at the hashed name being served. Unlike main, test does not set mp_theater_override in its
# server.cfg or playlist - deliberately, because both are re-applied every map change and would
...
# of the theater it has to match, both come from manifest.json on the fastdl host at map start. That
# is what lets the content image be republished without rebuilding or redeploying this one.
